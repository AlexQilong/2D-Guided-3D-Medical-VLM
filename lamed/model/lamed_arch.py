...
from lamed.model.loss import BCELoss, BinaryDiceLoss
...
class LamedMetaModel:
    def __init__(self, config):
        ...
        # Simple region projector to condition language on segmentation summary
        # Maps a scalar or small vector summary to LM hidden size
    ...
    def initialize_vision_modules(self, model_args):
        def _parse_tuple3(val):
            ...
            raise ValueError(f"image/patch size expects 3 ints, got: {val}")

        self.config.image_channel = model_args.image_channel
        self.config.image_size = _parse_tuple3(model_args.image_size)
        self.config.patch_size = _parse_tuple3(model_args.patch_size)
    ...
    def prepare_inputs_for_multimodal(
        ...
        if vision_tower is None or images is None or input_ids.shape[1] == 1:
            ...
            if region_condition is not None:
                try:
                    # Expect region_condition shape (B, *) -> reduce to (B,1)
                    if region_condition.dim() > 2:
                        region_condition = region_condition.view(region_condition.size(0), -1)
                    if region_condition.size(1) != 1:
                        region_condition = region_condition.mean(dim=1, keepdim=True)
                    region_vec = self.region_projector(region_condition.float())  # (B, hidden)
                    image_features = image_features + region_vec.unsqueeze(1)
                except Exception:
                    pass
